...
class UomApi(ViewSet):
    ...
    def list(self, request: Request) -> Response:
        ...
        uom_serializer = self.serializer_class(uoms, many=True)
        return HttpUtil.success_response(data=uom_serializer.data)

    def create(self, request: Request) -> Response:
        uom_serializer = self.serializer_class(data=request.data)
        if not uom_serializer.is_valid():
            return HttpUtil.error_response(uom_serializer.errors)
        uom_serializer.save()
        return HttpUtil.success_response(
            data=uom_serializer.data, code=status.HTTP_201_CREATED, message="created"
        )

    def retrieve(self, request: Request, uid: str = None) -> Response:
        try:
            uom = Uom.objects.get(uid=uid)
            uom_serializer = self.serializer_class(uom, many=False)
            return HttpUtil.success_response(data=uom_serializer.data)
        except Uom.DoesNotExist:
            return HttpUtil.error_response(message="uom not found!")

    def update(self, request: Request, uid: str = None) -> Response:
        try:
            uom = Uom.objects.get(uid=uid)
            uom_serializer = self.serializer_class(uom, data=request.data)
            if not uom_serializer.is_valid():
                return HttpUtil.error_response(uom_serializer.errors)
            uom_serializer.save()
            return HttpUtil.success_response(
                data=uom_serializer.data, message="updated."
            )
        except Uom.DoesNotExist:
            return HttpUtil.error_response(message="uom not found!")
    ...
    def delete(self, request: Request, uid: str = None) -> Response:
        try:
            uom = Uom.objects.get(uid=uid)
            uom.delete()
            return HttpUtil.success_response(message="uom deleted.")
        except Uom.DoesNotExist:
            return HttpUtil.error_response(message="uom not found!")
